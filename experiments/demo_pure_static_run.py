...
print("Starting Pure Static Optimization Demo...")

env = Environment(bounds={'x': (0, 100), 'y': (0, 100), 'z': (0, 50)})
# Central static threat
env.add_threat([50, 50, 0], radius=15, strength=10000.0)
env.add_threat([20, 40, 0], radius=10, strength=10000.0)
env.add_threat([80, 40, 0], radius=10, strength=10000.0)
...
fake_results = [{
    'algorithm': 'D-GTCPSO (Static Test)',
    'best_paths': paths_array,
    'path_times': path_times
}]

# First threat was static (0.0s)
fig = create_3d_scene(env, fake_results, scenario_name='Pure Static Threat Avoidance Test', threat_times=[0.0, 0.0, 0.0])
out_path = os.path.abspath('demo_pure_static.html')
